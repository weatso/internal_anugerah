...
export default function DocumentBuilderPage() {
  ...
  // docType state DIHAPUS. Sistem dikunci ke QUOTATION.
  ...
  async function handleSave() {
    ...
    setSubmitting(true)
    try {
      // SISTEM DIKUNCI: Semua dokumen baru adalah QUOTATION
      const divCode = effectiveEntity?.name.substring(0, 3).toUpperCase() || 'AV'
      const docNumber = `QUO/${divCode}/${new Date().getFullYear()}/${Math.floor(1000 + Math.random() * 9000)}`

      const { data: doc, error: docErr } = await supabase.from('commercial_documents').insert({
        entity_id: effectiveEntity?.id,
        client_id: clientId,
        doc_type: 'QUOTATION', // HARCODED ABSOLUT
      ...
      toast.success('Penawaran (Quotation) berhasil dibuat.')
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-8">
      ...
      <div className="border-b pb-6" style={{ borderColor: 'var(--border-subtle)' }}>
        ...
        <h1 className="text-2xl font-black tracking-tight" style={{ color: 'var(--text-primary)' }}>Pembuatan Penawaran (Quotation)</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="flex justify-end">
            <button onClick={handleSave} disabled={submitting}
              ...
              {submitting ? 'Menyimpan...' : '✓ Simpan Penawaran'}
            </button>
          </div>
        </div>
      </div>
