    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <h1 className="text-[--color-text-primary] text-2xl font-black tracking-tighter">Arsip Dokumen Komersial</h1>
        </div>
          ...
          <tbody className="divide-y divide-white/5">
            {documents.map(doc => (
              <tr key={doc.id} className="hover:bg-white/[0.02]">
                ...
                <td className="px-6 py-4">
                  <p className="font-bold text-[--color-text-primary] mb-0.5">{doc.clients?.company_name}</p>
                  <p className="text-gray-400 text-xs">{doc.title}</p>
                </td>
                <td className="px-6 py-4 text-xs text-gray-400">{doc.entities?.name}</td>
                <td className="px-6 py-4 text-right font-mono font-bold text-[--color-text-primary]">{formatRupiah(doc.grand_total)}</td>
                <td className="px-6 py-4 text-center">
                  <span className={`text-[9px] px-2 py-1 rounded-sm uppercase tracking-widest font-bold ${doc.status === 'DRAFT' ? 'bg-white/10 text-[--color-text-primary]' : doc.status === 'PAID' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-blue-500/10 text-blue-500'}`}>
                    {doc.status}
                  </span>
                </td>
                <td className="px-6 py-4 text-right">
                  <button onClick={() => openPdf(doc.id)} className="text-[#C5A028] hover:text-[--color-text-primary] bg-[#C5A028]/10 p-2 rounded-sm transition-colors" title="Lihat PDF">
                    <FileText size={16} />
                  </button>
                </td>
              </tr>
